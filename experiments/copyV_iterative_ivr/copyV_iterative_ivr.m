%% copyV_iterative_ivr — copyU with iterative IVR subspace estimation
...
[Ahat,Bhat,Phat,Rhat,Sigma_eps,stats] = control_aware_iterative_ivr_varx(y_off,u_off,ell,tracked);
...
fprintf('\ncopyV iterative-IVR smooth-noise process LV-SMPC results\n');
...
out.schema_version = 'copyV_iterative_ivr_v1';
out.description = 'copyT extension with raised-cosine time-varying Gaussian noise scales';
...
out.sw = sw; out.se = se; out.sigma_w_profile = sigma_w_profile; out.sigma_e_profile = sigma_e_profile;
...
save(fullfile(results_dir,'copyV_iterative_ivr_data.mat'), '-struct', 'out', '-v7.3');

fid = fopen(fullfile(results_dir,'copyV_iterative_ivr_metrics.txt'), 'w');
fprintf(fid, 'copyV_iterative_ivr metrics\n');
...
title(tlo, sprintf('copyV iterative-IVR smooth-noise PredVARX-SMPC (p=%d, ell=%d, tracked projection error %.1e)', p, ell, stats.tracked_projection_error));
print(fig, fullfile(results_dir,'copyV_iterative_ivr_fig'), '-dpng', '-r160');
