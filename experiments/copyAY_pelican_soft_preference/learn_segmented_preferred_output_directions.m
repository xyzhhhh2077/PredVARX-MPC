...
% CDR contract: y(:,k+1) is driven by u(:,k) (the observation returned by
% step k+1 reflects x_{k+1} = A x_k + B2 u_k), so the input side of the
% transition (y(:,k), u(:,k)) -> y(:,k+1) uses column k, not k+1.
