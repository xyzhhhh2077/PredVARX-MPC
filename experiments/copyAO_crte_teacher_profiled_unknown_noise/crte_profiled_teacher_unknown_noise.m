function [Ahat,Bhat,P,R,Sigma_eps,stats] = crte_profiled_teacher_unknown_noise(y,u,ell,tracked,opt)
%CRTE_PROFILED_TEACHER_UNKNOWN_NOISE Evaluate the complete profiled CRTE teacher.
%
...
tr=1:ntr; va=ntr+1:T;
assert(ntr>Np+ell+m+20,'Training segment too short.');
...
% Exact OLS FWL residual maker on training transitions (no ridge projector).
base=[yc(tracked,tr(1:end-1));uc(:,tr(1:end-1))];
...
function C=crossfit_free_residual_cov(Yp,uc,tr,ridge)
idx=tr(1:end-1); n=numel(idx); fold=mod(1:n,2)+1; E=[];
for f=1:2
    fit=fold~=f; tst=fold==f;
    Phi=[Yp(:,idx(fit));uc(:,idx(fit))]; Tar=Yp(:,idx(fit)+1);
    Th=(Phi*Phi'+ridge*eye(size(Phi,1)))\(Phi*Tar');
    Phit=[Yp(:,idx(tst));uc(:,idx(tst))]; Et=Yp(:,idx(tst)+1)-Th'*Phit; E=[E,Et]; %#ok<AGROW>
end
