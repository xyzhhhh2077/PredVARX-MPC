%% copyAB_opinion_fixes — fixed tracked axes + noise-optimal free extractor
% Synthetic industrial-process validation for high-dimensional process data.
% Baseline main/ and copyX are untouched. This copy splits the latent design:
% tracked loading/extractor columns are hard-fixed; only free latent directions
% use a true sensor-noise-covariance-weighted dual, followed by a full VARX refit.
clear; clc; close all;
...
sensor_noise_mode = 'heteroscedastic_correlated';
...
%% Declared true sensor-noise covariance
% The isotropic alternative Sigma_n=se^2*I is covered by the focused test
% and must collapse to R=P. This experiment deliberately uses a positive-
% definite heteroscedastic/correlated covariance so a nontrivial free-space
% oblique extractor has a genuine minimum-noise interpretation.
...
%% Identify split tracked/free PredVARX model
...
% Opinion 6 offline cross-covariance diagnostic (does not touch the CL loop).
% Documents that R'*o=0 samplewise does NOT imply Cov(z,o)=0, so the SMPC
% drop-cross Sigma_y = P*Sigma_z*P' + Sigma_obs may miss nonzero cross blocks.
...
Sy_off = (yc_off * yc_off') / den_off;
Sy_off = (Sy_off + Sy_off') / 2;
...
fprintf(['Opinion6 cross-cov offline: full_rel_err=%.3e, drop_cross_rel_err=%.3e, ' ...
    '||Sigma_zo||_F=%.3e, max|R''*o|=%.3e\n'], ...
    full_cross_rel_err, drop_cross_rel_err, norm(Szo_xcov, 'fro'), ...
    max(abs(Rhat' * o_xcov), [], 'all'));
...
% Opinion 8: chance-constraint certification grade per step.
% 'qp' = successful chance-constrained QP; 'uncertified_fallback' = heuristic
% sat(u_mean) path with NO chance-constraint guarantee (even if mean is safe).
...
infeasible_count = 0;
last_sigma_obs_diag = struct();  % opinion 7: optional support diagnostics

for k = 1:T_cl
    % Preserve the smooth global envelope while retaining the declared
    % cross-sensor covariance shape.
    ...
    if obs_count >= 5
        ...
        % Optional support diagnostics (rank/trace declared vs residual).
        % Does not change the online Sigma_obs used by SMPC.
        try
            [r_o, r_n, tr_o, tr_on, note7] = sigma_obs_support_diag( ...
                Owin, Sigma_n, p, ell);
            last_sigma_obs_diag.k = k;
            last_sigma_obs_diag.rank_o = r_o;
            last_sigma_obs_diag.rank_n = r_n;
            last_sigma_obs_diag.trace_o = tr_o;
            last_sigma_obs_diag.trace_on = tr_on;
            last_sigma_obs_diag.p_minus_ell = p - ell;
            last_sigma_obs_diag.note = note7;
        catch
            % Diagnostic library missing or failed: never break closed loop.
        end
    end
    ...
    try
        ...
        costJ(k) = info.cost;
        % Opinion 8: only a successful chance-constrained QP is certified.
        cc_cert_level{k} = 'qp';

    catch ME
        infeasible_count = infeasible_count + 1;
        % Safe input-bound fallback: sat(u_mean). Input-feasible only —
        % does NOT inherit the QP chance-constraint certificate.
        uk = min(max(model.u_mean, u_min), u_max);
        fb = fallback_certify_step(yk, model, opt, uk);
        exitflag(k) = fb.exitflag;                 % -1
        max_cc_violation(k) = fb.max_cc_violation; % NaN: no A_ch*U-b_ch
        cc_cert_level{k} = fb.cc_cert_level;       % 'uncertified_fallback'
        ...
        if infeasible_count <= 3
            fprintf(['QP uncertified fallback at k=%d: %s | %s\n'], ...
...
% Opinion 8: split QP-certified vs uncertified fallback; fallbacks must NOT
% be counted as chance-constraint-guaranteed successes.
...
% Only QP steps contribute to chance residual statistics.
...
fprintf('\ncopyAB split tracked/free noise-optimal PredVARX-SMPC results\n');
...
fprintf('free oblique norm = %.3e, true-noise objective %.6e -> %.6e (gain %.6e)\n', ...
    stats.free_oblique_norm,stats.free_noise_baseline, ...
    stats.free_noise_objective,stats.free_noise_improvement);
...
fprintf('cc cert: qp_certified_rate=%.4f (%d), uncertified_fallback_rate=%.4f (%d) [fallback is NOT chance-constraint certified]\n', ...
    qp_certified_rate, qp_certified_count, uncertified_fallback_rate, uncertified_fallback_count);
fprintf('constraint active rate (QP steps only) = %.4f (residual > -1e-3)\n',constraint_active_rate);
...
out.schema_version = 'copyAB_opinion_fixes_v1';
out.description = 'fixed tracked axes plus true-noise-optimal free-subspace dual extractor';
...
out.sw = sw; out.se = se; out.sensor_noise_mode = sensor_noise_mode; out.Sigma_n = Sigma_n; out.sensor_rel = sensor_rel; out.Corr_n = Corr_n; out.sigma_w_profile = sigma_w_profile; out.sigma_e_profile = sigma_e_profile;
...
out.cc_cert_level = cc_cert_level;
out.fallback_det_mean_violation = fallback_det_mean_violation;
...
out.sigma_obs_support_diag = last_sigma_obs_diag;  % opinion 7 last online diag
...
out.qp_certified_count = qp_certified_count; out.qp_certified_rate = qp_certified_rate;
out.uncertified_fallback_count = uncertified_fallback_count;
out.uncertified_fallback_rate = uncertified_fallback_rate;
save(fullfile(results_dir,'copyAB_opinion_fixes_data.mat'), '-struct', 'out', '-v7.3');

fid = fopen(fullfile(results_dir,'copyAB_opinion_fixes_metrics.txt'), 'w');
fprintf(fid, 'copyAB_opinion_fixes metrics\n');
...
fprintf(fid, 'free_noise_baseline %.12e\n', stats.free_noise_baseline);
fprintf(fid, 'free_noise_objective %.12e\n', stats.free_noise_objective);
fprintf(fid, 'free_noise_improvement %.12e\n', stats.free_noise_improvement);
fprintf(fid, 'free_noise_is_declared_Sigma_n %d\n', stats.free_noise_is_declared_Sigma_n);
fprintf(fid, 'isotropic_collapsed_to_R_eq_P %d\n', stats.isotropic_collapsed_to_R_eq_P);
...
fprintf(fid, 'qp_certified_count %d\n', qp_certified_count);
fprintf(fid, 'qp_certified_rate %.12f\n', qp_certified_rate);
fprintf(fid, 'uncertified_fallback_count %d\n', uncertified_fallback_count);
...
title(ax5, sprintf('QP chance residual (QP only): success=%.3f, active=%.3f, uncertified_fallback=%d, max=%.2e', ...
    qp_success_rate, constraint_active_rate, uncertified_fallback_count, max_recorded_qp_constraint));

linkaxes([ax1 ax2 ax3 ax4 ax5], 'x');
title(tlo, sprintf('copyAB split tracked/free PredVARX-SMPC (p=%d, ell=%d, free oblique %.1e)', p, ell, stats.free_oblique_norm));
print(fig, fullfile(results_dir,'copyAB_opinion_fixes_fig'), '-dpng', '-r160');
