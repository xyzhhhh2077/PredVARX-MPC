function report = run_cert_config(cfg, plant, data)
% RUN_CERT_CONFIG Closed-loop with recovery modes:
%   'none'              - uncertified sat(u_mean) on primary fail
%   'soft_relaxed'      - old soft recovery (NOT original alpha)
%   'original_alpha'    - recovery only if N-step original-alpha rows pass
%
% Certificate rules:
%   PRIMARY success: full planned U from N-step QP is original-alpha certified
%     by construction if max(A_ch*U-b_ch)<=tol (recorded from solver info).
%   RECOVERY single-uk: constant-hold extension of uk over N must pass
%     original-alpha rows (certify_input_original_alpha).
...
n_short=0; n_onestep=0; n_prev=0; n_umean=0;
n_soft_also_orig=0;
...
for k=1:T_cl
    ...
    try
        ...
        uk=U(1:m);
        % Primary N-step plan: original-alpha certified by QP rows on full U.
        max_viol = max(info.A_ch*U - info.b_ch);
        prim_ok = (info.exitflag>0) && (max_viol <= 1e-7);
        level{k}='qp_primary';
        stage{k}='primary';
        n_primary=n_primary+1;
        orig_cert_flag(k)=prim_ok;
        if ~prim_ok
            % Should be rare; force recovery path under original_alpha mode.
            error('centered_smpc_step:PrimaryRowFail','primary U failed row residual check');
        end
    catch
        pf=pf+1;
        switch cfg.recovery_mode
            ...
            case 'original_alpha'
                fb=original_alpha_recovery(yk,rk,model,opt,u_prev);
                uk=fb.uk;
                level{k}=fb.cc_cert_level;
                stage{k}=fb.stage_class;
                if fb.success
                    n_orig_rec=n_orig_rec+1;
                    orig_cert_flag(k)=true;
                    switch fb.stage_class
                        case 'short_horizon_original_alpha', n_short=n_short+1;
                        case 'one_step_original_alpha', n_onestep=n_onestep+1;
                        case 'backup_prev', n_prev=n_prev+1;
                        case 'backup_umean', n_umean=n_umean+1;
                    end
                else
                    n_unc=n_unc+1; orig_cert_flag(k)=false;
                end
...
report.primary_fail=pf;
report.n_primary=n_primary;
report.n_orig_rec=n_orig_rec;
report.n_soft_rel=n_soft_rel;
report.n_unc=n_unc;
...
report.n_short=n_short; report.n_onestep=n_onestep;
report.n_prev=n_prev; report.n_umean=n_umean;
report.n_soft_also_orig=n_soft_also_orig;
...
report.cert_def = ['primary: full U row residual; recovery uk: constant-hold ' ...
    'original-alpha rows'];
end
