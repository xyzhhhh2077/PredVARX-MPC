...
% Fixed tracked axes + free dual under empirical free-coordinate total
% covariance metric. True sensor-noise Sigma_n is UNKNOWN to the algorithm
% and is never passed into the identifier.
%
% Honest claim boundary:
% - alpha=1 free dual is conditional optimum under the empirical-total-
%   covariance metric on free coordinates Yp=Nperp'*yc.
% - NOT sensor-noise optimum theorem.
% - NOT PredVAR minimum innovation covariance theorem.
% Baseline main/, copyX, copyAA are untouched.
clear; clc; close all; addpath(fileparts(mfilename('fullpath')));
rng(20260710,'twister');  % same seed as copyAA for fair plant comparison
...
y_max = 2.00;
alpha_joint = 0.10;
oblique_alpha = 1;   % empirical-metric conditional optimum (not interpolation)
...
%% Identify WITHOUT Sigma_n
[Ahat,Bhat,Phat,Rhat,Sigma_eps,stats] = split_empirical_cov_ivr_varx( ...
    y_off,u_off,ell,tracked,oblique_alpha);
...
% The algorithm does not know true Sigma_n or the plant scale se.
% Initialize the observation/residual covariance from the offline data only.
% This is an empirical total-residual proxy, not a sensor-noise covariance.
IminusPR_off = eye(p)-Phat*Rhat';
obs_off = IminusPR_off*(y_off-stats.y_mean);
...
for k = 1:T_cl
    ...
    if obs_count >= 5
        ...
        % Empirical residual proxy only; no true Sigma_n shape or scale.
        Ocov = (Owin*Owin')/max(obs_count,1);
        Ocov = (Ocov+Ocov')/2;
        model.Sigma_obs = Ocov + 1e-8*eye(p);
    end
...
% cover = time-fraction proxy of warm steps with successful QP exitflag>0
cover = mean(exitflag(warm) > 0);

fprintf('\ncopyAM split empirical-total-cov free dual PredVARX-SMPC results\n');
fprintf('metric = %s (NOT sensor-noise optimum; residual proxy is empirical only; NOT PredVAR min innovation)\n', stats.metric_name);
fprintf('chance_covariance = tracked-only P*Sigma_z*P'' (Sigma_obs proxy diagnostic only)\n');
fprintf('oblique_alpha = %.3f (1=empirical free metric conditional optimum)\n', stats.oblique_alpha);
fprintf('uses_true_Sigma_n = %d\n', stats.uses_true_Sigma_n);
...
fprintf('free oblique norm = %.3e, free emp-cov objective %.6e -> %.6e (gain %.6e)\n', ...
    stats.free_oblique_norm,stats.free_emp_cov_baseline, ...
    stats.free_emp_cov_objective,stats.free_emp_cov_improvement);
...
out.schema_version = 'copyAM_tracked_cov_only_v1';
out.description = ['fixed tracked axes + free dual under empirical free-coordinate ' ...
    'total covariance metric; no oracle Sigma_n'];
...
out.Sigma_n_plant = Sigma_n_plant;  % plant only; not used by identifier
out.sensor_rel = sensor_rel; out.Corr_n = Corr_n;
out.sigma_w_profile = sigma_w_profile; out.sigma_e_profile = sigma_e_profile;
out.noise_cycle = noise_cycle; out.sw_min = sw_min; out.sw_max = sw_max;
...
out.oblique_alpha = oblique_alpha;
...
out.costJ = costJ; out.estimated_sigma_eps = estimated_sigma_eps;
...
save(fullfile(results_dir,'copyAM_tracked_cov_only_data.mat'), '-struct', 'out', '-v7.3');

fid = fopen(fullfile(results_dir,'copyAM_tracked_cov_only_metrics.txt'), 'w');
fprintf(fid, 'copyAM_tracked_cov_only metrics\n');
fprintf(fid, 'metric_name %s\n', stats.metric_name);
fprintf(fid, 'uses_true_Sigma_n %d\n', stats.uses_true_Sigma_n);
fprintf(fid, 'oblique_alpha %.12f\n', stats.oblique_alpha);
...
fprintf(fid, 'free_emp_cov_baseline %.12e\n', stats.free_emp_cov_baseline);
fprintf(fid, 'free_emp_cov_objective %.12e\n', stats.free_emp_cov_objective);
fprintf(fid, 'free_emp_cov_improvement %.12e\n', stats.free_emp_cov_improvement);
...
title(tlo, sprintf('copyAM empirical free dual (alpha=%.2f, free oblique %.1e)', ...
    oblique_alpha, stats.free_oblique_norm));
print(fig, fullfile(results_dir,'copyAM_tracked_cov_only_fig'), '-dpng', '-r160');
