...
% Same-seed plant compare of copyAM (empirical free dual, no Sigma_n)
...
al_dir = fullfile(repo,'experiments','copyAM_tracked_cov_only');
aa_dir = fullfile(repo,'experiments','copyAA_split_control_free_oblique');
assert(exist(al_dir,'dir')==7, ['copyAM missing: ' al_dir]);
assert(exist(aa_dir,'dir')==7, ['copyAA missing: ' aa_dir]);

fprintf('=== Running copyAM (empirical free dual) ===\n');
addpath(al_dir);
run(fullfile(al_dir,'copyAM_tracked_cov_only.m'));
% rebuild absolute paths after runner clear
repo = ['E:', filesep, 'academic_files', filesep, 'phd-learning', filesep, ...
    char([20195 30721]), filesep, 'PredVARX-MPC'];
al_dir = fullfile(repo,'experiments','copyAM_tracked_cov_only');
aa_dir = fullfile(repo,'experiments','copyAA_split_control_free_oblique');
al_mat_path = fullfile(al_dir,'results','copyAM_tracked_cov_only_data.mat');
al_metrics_path = fullfile(al_dir,'results','copyAM_tracked_cov_only_metrics.txt');
assert(exist(al_mat_path,'file')==2, ['AM mat missing: ' al_mat_path]);
...
assert(exist(al_mat_path,'file')==2, ['AM mat missing after AA: ' al_mat_path]);

AM = load(al_mat_path);
AA = load(aa_mat_path);

out_txt = fullfile(al_dir,'results','copyAM_vs_copyAA_compare.txt');
fid = fopen(out_txt,'w');
fprintf(fid,'copyAM vs copyAA same-seed plant comparison\n');
...
fprintf(fid,'note AM free metric is empirical total free-cov; AA is declared Sigma_n\n');
