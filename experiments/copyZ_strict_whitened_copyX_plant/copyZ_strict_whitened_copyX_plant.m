...
[Ahat,Bhat,Phat,Pbarhat,Rhat,Rbarhat,~,Sigma_eps,Sigma_ebar,~,~,~,info] = ...
    predvarx_identify_strict_whitened(y_off,u_off,ell);
stats = info;
...
stats.tracked_projection_error = norm(Phat*Rhat'*Ec-Ec,'fro');
stats.tracked_oblique_error = stats.tracked_projection_error;
stats.dual_error = norm(Rhat'*Phat-eye(ell),'fro');
stats.pr_asymmetry = norm(Phat*Rhat'-(Phat*Rhat')','fro');
stats.cond_dual_gram = cond(Rhat'*Phat);
stats.oblique_alpha = NaN;
stats.reconstruction_residual = norm((y_off-mean(y_off,2))-Phat*(Rhat'*(y_off-mean(y_off,2))),'fro')/norm(y_off-mean(y_off,2),'fro');
...
model.Sigma_obs = Pbarhat*Sigma_ebar*Pbarhat';
model.Sigma_obs = (model.Sigma_obs+model.Sigma_obs')/2;
...
out.sw = sw; out.se = se; out.oblique_alpha = NaN; out.sigma_w_profile = sigma_w_profile; out.sigma_e_profile = sigma_e_profile;
...
out.Ahat = Ahat; out.Bhat = Bhat; out.Phat = Phat; out.Pbarhat=Pbarhat; out.Rhat = Rhat; out.Rbarhat=Rbarhat; out.Sigma_eps = Sigma_eps; out.Sigma_ebar=Sigma_ebar; out.stats = stats;
...
save(fullfile(results_dir,'copyZ_strict_whitened_copyX_plant_data.mat'), '-struct', 'out', '-v7.3');
