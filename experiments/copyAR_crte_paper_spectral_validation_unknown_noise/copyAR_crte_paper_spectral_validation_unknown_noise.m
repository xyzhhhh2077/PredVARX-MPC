...
% Paper CRTE experimental algorithm (draft Sec. 3.3-3.4), not profiled teacher:
% - fixed spectral surrogate ACRTE = Ntr(S)+Ntr(A_T)-Ntr(C_n), paper Ntr
% - C_n from one-step free-output residual covariance proxy (unknown Sigma_n)
% - mu continuum only (alpha=beta=1 as written); validation-only selection
% - outer task/noise/reach gates; rebuild selected mu on all offline data
% - Coverage = warm-horizon QP-success time fraction; NOT chance certificate
% - No recursive feasibility / stability / global optimality claim
clear; clc; close all; addpath(fileparts(mfilename('fullpath')));
% Drop broken MOSEK shims once for the whole closed-loop run.
parts = strsplit(path, pathsep);
path(strjoin(parts(~contains(lower(parts),'mosek')), pathsep));
...
%% Plant sensor-noise: data generation ONLY; algorithm never receives Sigma_n
...
fprintf('\ncopyAR paper CRTE spectral+validation+unknown residual proxy (free complement only) results\n');
...
out.schema_version = 'copyAR_v2_paper_spectral_val_unknown_Sn';
out.description = ['paper CRTE fixed spectral surrogate Sec.3.3-3.4; ' ...
    'unknown residual proxy; paper Ntr; validation-only selection; alpha=beta=1'];
...
save(fullfile(results_dir,'copyAR_crte_paper_spectral_validation_unknown_noise_data.mat'),'-struct','out','-v7.3');

fid = fopen(fullfile(results_dir,'copyAR_crte_paper_spectral_validation_unknown_noise_metrics.txt'),'w');
fprintf(fid,'copyAR_crte_paper_spectral_validation_unknown_noise metrics\n');
...
title(tlo,sprintf('copyAR paper CRTE spectral+val+unknown residual proxy: mu=%.2f alpha=%.2f beta=%.2f val NRMSE=%.3f (valid %d/%d)', ...
...
print(fig,fullfile(results_dir,'copyAR_crte_paper_spectral_validation_unknown_noise_fig'),'-dpng','-r160');
close(fig);