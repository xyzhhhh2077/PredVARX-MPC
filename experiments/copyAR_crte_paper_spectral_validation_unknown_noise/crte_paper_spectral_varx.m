function [Ahat,Bhat,P,R,Sigma_eps,stats] = crte_paper_spectral_varx(y,u,ell,tracked,opt)
%CRTE_PAPER_SPECTRAL_VARX Paper CRTE fixed spectral surrogate (draft Sec. 3.3-3.4).
% Executable path (NOT profiled teacher (23), NOT min-teacher copyAO):
%   1) free-complement metric C_mu = (1-mu)*Sigma_perp + mu*tau_G*G
%   2) fixed free-source S_yu, A_T, and UNKNOWN-noise residual proxy
%      C_n = one-step free-output residual covariance on training data
%   3) paper Ntr(A) = A / max{|tr(A)|, eps}
%   4) ACRTE = Ntr(S_yu) + Ntr(A_T) - Ntr(C_n)   (alpha=beta=1)
%   5) ACRTE V = C_mu V, V'*C_mu*V = I via symmetric whitening
%   6) complete dual basis + refit VARX per mu candidate
%   7) outer gates: task / noise / reachability / dual / spectral radius
%   8) validation-only one-step NRMSE on controlled outputs
%   9) rebuild selected mu on ALL offline data, freeze model
%
% True sensor-noise Sigma_n is NEVER accepted. Residual proxy is not Sigma_n.
%
% Optional: mu_grid, val_fraction, ridge, task_gate_fraction, noise_gate_factor,
%           reach_gate_fraction, reach_horizon, G, Ru
...
assert(r>0,'CRTE requires at least one free latent coordinate.');
...
if isempty(opt.Ru)
    ...
    Ru = (opt.Ru+opt.Ru')/2;
    assert(isequal(size(Ru),[m m]) && min(eig(Ru))>0,'Ru must be m-by-m SPD.');
    Ru_inv = Ru\eye(m);
end
...
% Fixed free-output prediction content S_yu + residual noise proxy (unknown Sigma_n).
...
S_yu = (S_yu+S_yu')/2;
Efree = Ycur-Ypred;
C_n = (Efree*Efree')/max(size(Efree,2),1);
C_n = (C_n+C_n')/2 + ridge*eye(d);

% Exact fixed-space FWL task matrices (one-step future tracked outputs).
...
% Paper Ntr and ACRTE with alpha=beta=1.
NtrS = paper_ntr(S_yu);
NtrA = paper_ntr(A_T);
NtrN = paper_ntr(C_n);
Acrte0 = NtrS + NtrA - NtrN;
Acrte0 = (Acrte0+Acrte0')/2;
...
if ~any(valid)
    warning('crte_paper_spectral_varx:NoCandidatePassed', ...
        'No candidate passed all gates; using min validation NRMSE among stable dual candidates.');
...
% Rebuild selected mu on ALL offline data.
...
E_f = Ycur_f-Ypred_f;
C_n_f = (E_f*E_f')/size(E_f,2); C_n_f=(C_n_f+C_n_f')/2+ridge*eye(d);
...
Acrte_f = paper_ntr(S_f)+paper_ntr(A_f)-paper_ntr(C_n_f);
Acrte_f = (Acrte_f+Acrte_f')/2;
Mgev=Cinvhalf*Acrte_f*Cinvhalf; Mgev=(Mgev+Mgev')/2;
...
stats.selected_alpha = 1;
stats.selected_beta = 1;
...
stats.noise_object = 'one-step free-output residual covariance proxy (unknown Sigma_n)';
stats.objective = 'paper fixed CRTE spectral surrogate Sec.3.3; validation-only selection Sec.3.4; unknown-noise residual proxy';
stats.selection = 'validation-only one-step NRMSE on controlled outputs after gates';
stats.ntr_mode = 'paper_trace_normalize';
stats.C_mu = C_f;
stats.tau_G = tau_full;
stats.S_yu = S_f;
stats.A_T = A_f;
stats.C_n = C_n_f;
stats.Sigma_noise_proxy = C_n_f;
