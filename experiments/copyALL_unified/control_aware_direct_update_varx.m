function [Ahat,Bhat,P,R,Sigma_eps,stats] = control_aware_direct_update_varx(y,u,ell,tracked,oblique_alpha)
% CONTROL_AWARE_DIRECT_UPDATE_VARX
% Control-aware model with direct predictable-direction replacement in raw
% centered output coordinates. This variant deliberately does NOT apply the
% paper normalization Y*=Y*U*D^(-1/2), nor any de-normalization afterward.
...
if r == 0
    ...
    % Initialize in raw centered complement coordinates; no whitening/normalization.
    [U0,~,~] = svd(Yp,'econ');
    ...
    for iter = 1:max_iter
        ...
        % Direct replacement update in the same raw complement coordinates.
...
stats.update_rule = 'direct_eigenspace_replacement';
