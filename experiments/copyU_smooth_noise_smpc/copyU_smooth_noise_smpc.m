%% copyU_smooth_noise_smpc — copyT with smooth time-varying Gaussian noise scales
...
fprintf('\ncopyU smooth-noise process LV-SMPC results\n');
...
out.schema_version = 'copyU_smooth_noise_smpc_v1';
...
save(fullfile(results_dir,'copyU_smooth_noise_smpc_data.mat'), '-struct', 'out', '-v7.3');

fid = fopen(fullfile(results_dir,'copyU_smooth_noise_smpc_metrics.txt'), 'w');
fprintf(fid, 'copyU_smooth_noise_smpc metrics\n');
...
title(tlo, sprintf('copyU smooth-noise PredVARX-SMPC (p=%d, ell=%d, tracked projection error %.1e)', p, ell, stats.tracked_projection_error));
print(fig, fullfile(results_dir,'copyU_smooth_noise_smpc_fig'), '-dpng', '-r160');
