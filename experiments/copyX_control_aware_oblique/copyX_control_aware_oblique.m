%% copyX_control_aware_oblique — copyV subspace with an oblique dual extractor
...
oblique_alpha = 0.02;
[Ahat,Bhat,Phat,Rhat,Sigma_eps,stats] = control_aware_oblique_ivr_varx(y_off,u_off,ell,tracked,oblique_alpha);
...
fprintf('\ncopyX control-aware oblique-IVR smooth-noise process LV-SMPC results\n');
...
out.schema_version = 'copyX_control_aware_oblique_v1';
out.description = 'copyV control-aware subspace with covariance-weighted oblique dual extractor';
...
save(fullfile(results_dir,'copyX_control_aware_oblique_data.mat'), '-struct', 'out', '-v7.3');

fid = fopen(fullfile(results_dir,'copyX_control_aware_oblique_metrics.txt'), 'w');
fprintf(fid, 'copyX_control_aware_oblique metrics\n');
...
title(tlo, sprintf('copyX control-aware oblique PredVARX-SMPC (p=%d, ell=%d, oblique coverage %.1e)', p, ell, stats.tracked_oblique_error));
print(fig, fullfile(results_dir,'copyX_control_aware_oblique_fig'), '-dpng', '-r160');
