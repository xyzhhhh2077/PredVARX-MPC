"""Create a synchronized four-panel GIF from the verified copyBI SMPC run."""
...
DATA_PATH = RESULTS / "copyBI_smpc_figure_data.npz"
...
def main():
    ...
    fig.suptitle(
        "copyBI SMPC: six-face trajectory, disturbances, and realized stage cost",
        fontsize=14,
        fontweight="bold",
        y=0.985,
    )
    progress_text = fig.text(0.5, 0.952, "", ha="center", va="center", fontsize=9, color=BLACK)
    ...
    line3d, = ax3d.plot([], [], [], color=BLUE, linewidth=1.8, label="SMPC")
    ...
    ax_innov.set_title("d  Composite latent innovation and injected physical wind", loc="left", fontweight="bold")
    ...
    def update(frame):
        ...
        progress_text.set_text(
            f"t = {now:6.2f} s   |   frame {frame + 1:03d}/{FRAME_COUNT}   |   hard bound ±{hard_bound:.1f}"
        )
        for j in range(3):
    ...
    ani = animation.FuncAnimation(fig, update, frames=FRAME_COUNT, interval=FRAME_DURATION_MS, blit=False)
