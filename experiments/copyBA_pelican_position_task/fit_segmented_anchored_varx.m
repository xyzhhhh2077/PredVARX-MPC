...
%
% NOTE (copyAY): the A_T task-gram term is evaluated in LOW-RANK factored
% form.  The original copyAX implementation computed the T x T projection
%   H0 = base'/(base*base'+ridge*I)*base,  task_gram = Tfuture'*Tfuture,
%   A_T = W*M0*task_gram*M0*W'
% which is mathematically identical to
%   U = W*Tfuture' - (W*base')/(base*base'+ridge*I)*(base*Tfuture')
%   A_T = U*U'
% (verified numerically on random data).  The factored form avoids materializing
% T x T matrices and makes the fit tractable at T ~ 1e6 (Pelican real data).
...
if r>0
    ...
    % --- Task-gram A_T in factored form (see header note) ---
