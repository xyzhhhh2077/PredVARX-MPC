%% copyAW_boptest_soft_preference
% Apply the copyAU soft-preference CRTE construction to BOPTEST HVAC data.
clear; clc; close all; addpath(fileparts(mfilename('fullpath')));
here = fileparts(mfilename('fullpath'));
D = load(fullfile(here,'data','boptest_multizone_office_complex_air.mat'));

assert(isequal(size(D.y,1),30) && isequal(size(D.u,1),6), ...
    'Expected 30 BOPTEST outputs and 6 manipulated inputs.');
assert(size(D.y,2)==size(D.u,2) && size(D.y,2)==numel(D.segment_id), ...
    'BOPTEST sample counts do not match.');
assert(all(isfinite(D.y),'all') && all(isfinite(D.u),'all'), ...
    'BOPTEST snapshot contains non-finite values.');

segment_id = double(D.segment_id(:)');
train_mask = ismember(segment_id,1:2);
validation_mask = segment_id==3;
...
assert(all(y_scale>1e-10) && all(u_scale>1e-10), ...
    'A BOPTEST channel has zero training variance.');
...
p=30; m=6; q=2; ell=5; reach_horizon=18;
preference_strength=0.78; metric_mu=0.10; ntr_epsilon=10e-6;
% Every physical output remains represented. Temperature receives a mild soft
% preference over CO2; this is not a hard task-axis lock.
weights=[ones(15,1);0.85*ones(15,1)];
...
metrics_path=fullfile(results_dir,'copyAW_boptest_soft_preference_metrics.txt');
fid=fopen(metrics_path,'w'); cleaner=onCleanup(@() fclose(fid));
fprintf(fid,'copyAW BOPTEST soft preference offline validation\n');
fprintf(fid,'testcase multizone_office_complex_air\n');
...
fprintf(fid,'preference_weights temperature=1.0 co2=0.85\n');
...
fprintf(fid,'scope active_boptest_data_offline_one_step_prediction_only\n');
fprintf(fid,'closed_loop_mpc_run 0\nweather_disturbance_modeled 0\n');
clear cleaner;

save(fullfile(results_dir,'copyAW_boptest_soft_preference_data.mat'), ...
...
tt=(1:validation.transition_count)*15/60;
ax1=nexttile; plot(tt,validation.task_actual(1,:),'k', ...
    tt,validation.task_predicted(1,:),'b','LineWidth',1.0); grid on;
ylabel('task 1'); title('Held-out BOPTEST season: one-step task prediction');
legend('actual','copyAW prediction','Location','best');
ax2=nexttile; plot(tt,validation.task_actual(2,:),'k', ...
    tt,validation.task_predicted(2,:),'r','LineWidth',1.0); grid on;
ylabel('task 2'); title('Held-out BOPTEST season: one-step task prediction');
legend('actual','copyAW prediction','Location','best');
...
grid on; xlim([0 p+1]); xlabel('BOPTEST output index'); ylabel('direction contribution');
title('15 temperatures | 15 CO2 measurements');
legend('learned contribution','scaled soft preference','Location','best');
linkaxes([ax1 ax2],'x'); xlabel(ax2,'validation time (h)');
title(tl,sprintf('copyAW | BOPTEST 30 outputs -> 5 latent variables | task RMSE=%s', ...
    mat2str(validation.task_rmse',3)));
print(fig,fullfile(results_dir,'copyAW_boptest_soft_preference_fig.png'),'-dpng','-r160'); close(fig);
fprintf('copyAW BOPTEST complete: task RMSE=%s, persistence=%s, R2=%s\n', ...
