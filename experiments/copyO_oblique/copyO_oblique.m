%% copyO_oblique — SVD 双基斜投影 PredVARX-MPC 实验副本
...
[A0,B0,P0,Pbar0,R0,Rbar0,G0,Se0,~,F0,H0]=predvarx_identify_oblique(yo,uo,ell,beta_u,2,A,B,C,n,m,p);
Ac=F0;
...
xt=zeros(n,1); Sk=Se0; Ak=Ac; Pk=P0; Rk=R0; RbrS=Rbar0; Akh=A0; Bk=B0;
...
for k=1:T_cl
    ...
    if mod(k,Nr)==0 && k>0 && (k-tl)>=Nr
        [Akh,Bk,Pk,~,Rk,RbrS,G0,~,~,~,Ha]=predvarx_identify_oblique(yc,uc,ell,beta_u,2,A,B,C,n,m,p);
        Ak=Akh;
        for j=1:N_pred
            Mk{j}=Pk*Ex'*Ak^j;
            for i=0:j-1
                Nk{j}(:,i*m+1:(i+1)*m)=Pk*Ex'*Ak^(j-1-i)*Ha;
            end
        end
...
for k=1:T_cl
    ...
    if mod(k,Nr)==0 && k>0 && (k-tl)>=Nr
        ...
        [Akh,Bk,Pk,~,Rk,RbrS,G0,~,~,~,Ha]=predvarx_identify_oblique(yc_in,uc_in,ell,beta_u,2,A,B,C,n,m,p);
...
fprintf('C=%.3f(%d) Oblique=%.3f(%d) imp=%+.0f%% viol_red=%.0f%%\n',...
...
cC = [0.2 0.4 0.8];    % C 蓝
cC2 = [0.6 0.2 0.7];   % C 紫
cZ = [0.9 0.3 0.1];    % Oblique-SMPC 红
cZ2 = [0.9 0.6 0.1];   % Oblique-SMPC 橙
t_axis = 1:T_cl;
...
plot(t_axis, yZ(1,:), ':', 'Color',cZ, 'LineWidth',0.6, 'DisplayName','Oblique y_1');
plot(t_axis, yZ(2,:), 'Color',cZ, 'LineWidth',0.8, 'DisplayName',sprintf('Oblique y_2 (e=%.3f)',eZ));
...
hold on;
plot(t_axis, sZ_eps, 'Color',cZ, 'LineWidth',0.8, 'DisplayName',sprintf('Oblique sigma_eps (avg=%.3f)',mean(sZ_eps(500:end))));
plot(t_axis, sC_ebar, '--', 'Color',cC2, 'LineWidth',0.8, 'DisplayName',sprintf('C sigma_ebar (avg=%.3f)',mean(sC_ebar(500:end))));
plot(t_axis, sZ_ebar, '--', 'Color',cZ2, 'LineWidth',0.8, 'DisplayName',sprintf('Oblique sigma_ebar (avg=%.3f)',mean(sZ_ebar(500:end))));
...
plot(t_axis, uZ(1,:), 'Color',cZ, 'LineWidth',0.6, 'DisplayName','Oblique u_1');
plot(t_axis, uZ(2,:), '--', 'Color',cZ2, 'LineWidth',0.6, 'DisplayName','Oblique u_2');
...
plot(t_axis, costZ, 'Color',cZ, 'LineWidth',0.8, 'DisplayName',sprintf('Oblique (avg=%.2f)',mean(costZ(500:end))));
for s=1:5, ks=(s-1)*400; xline(ks,'Color',[.7 .7 .7],'HandleVisibility','off'); end
...
plot(t_axis, abs(yZ(2,:)-Rf(2,:)), 'Color',cZ, 'LineWidth',0.8, 'DisplayName',sprintf('Oblique (MAE=%.3f)',eZ));
for s=1:5, ks=(s-1)*400; xline(ks,'Color',[.7 .7 .7],'HandleVisibility','off'); end
