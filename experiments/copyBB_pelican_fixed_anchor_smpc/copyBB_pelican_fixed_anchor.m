%% copyBB_pelican_fixed_anchor
% CRTE-style fixed controlled directions on real Waterloo Pelican data.
% Flights 1:36 identify the model. Flights 37:54 are held out.
% The controlled directions are prescribed before fitting:
%   E_c = [standardized x position, standardized y position].
% Data do not re-learn or rotate E_c. Only the free complement is selected by
% the CRTE spectral construction inherited from copyAU/copyBA.
...
p=10; m=4; q=2; ell=5; metric_mu=0.10; ntr_epsilon=10e-6;
Eanchor=zeros(p,q); Eanchor(1,1)=1; Eanchor(2,2)=1;
assert(norm(Eanchor'*Eanchor-eye(q),'fro')<eps, ...
    'The prescribed x/y anchor must be orthonormal.');
...
% Training-only chance bound. It is fixed before closed-loop simulation.
train_task=Eanchor'*y_train;
task_bound=max(3.0,max(abs(train_task),[],'all'));

results_dir=fullfile(here,'results');
if ~exist(results_dir,'dir'), mkdir(results_dir); end
metrics_path=fullfile(results_dir,'copyBB_pelican_fixed_anchor_metrics.txt');
fid=fopen(metrics_path,'w'); cleaner=onCleanup(@() fclose(fid));
fprintf(fid,'copyBB fixed-anchor CRTE Pelican identification\n');
fprintf(fid,'training_source real_waterloo_pelican_flights_1_36\n');
fprintf(fid,'validation_source held_out_flights_37_54\n');
fprintf(fid,'controlled_directions prescribed_standardized_x_y_position\n');
fprintf(fid,'task_direction_learned_from_data 0\n');
fprintf(fid,'free_complement_crte_spectral 1\n');
fprintf(fid,'latent_dimension %d\nselected_mu %.12f\n',ell,fitstats.selected_mu);
...
fprintf(fid,'scope offline_identification_before_model_in_loop_control\n');
clear cleaner;

save(fullfile(results_dir,'copyBB_pelican_fixed_anchor_data.mat'), ...
    'Eanchor','metric_mu','ntr_epsilon','task_bound','fitstats','model', ...
    'validation','y_offset','y_scale','u_offset','u_scale','-v7.3');

fig=figure('Position',[80 80 1800 850],'Color','w','Visible','off');
tl=tiledlayout(fig,2,1,'TileSpacing','compact','Padding','compact');
tt=1:validation.transition_count;
for j=1:2
    ax=nexttile;
    plot(tt,validation.task_actual(j,:),'k',tt,validation.task_predicted(j,:), ...
        'Color',[0.03 0.49 0.55],'LineWidth',1.0); grid on;
    ylabel(sprintf('standardized %s',char('x'+j-1)));
    title(sprintf('Held-out one-step prediction: fixed %s-position direction',char('x'+j-1)));
    legend('actual','prediction','Location','best');
end
xlabel(ax,'validation transition index');
title(tl,sprintf('copyBB fixed-anchor identification | RMSE=%s | R^2=%s', ...
    mat2str(validation.task_rmse',3),mat2str(validation.task_r2',3)));
print(fig,fullfile(results_dir,'copyBB_pelican_fixed_anchor_prediction.png'),'-dpng','-r160');
close(fig);
fprintf('copyBB identification complete: RMSE=%s, persistence=%s, R2=%s\n', ...
