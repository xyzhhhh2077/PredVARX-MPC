"""copyBH input-cap ablation on the frozen copyBG Pelican experiment.
...
Only the inherited standardized motor-command box is swept. Identification,
chance tightening, objective, horizon, multirate lift, wind, and QP structure
are unchanged from copyBG.
"""
...
# The ±6 cap is an inherited copyBA/copyBE experiment-design boundary, not a
# Pelican hardware rating. copyBH changes only this experiment parameter. The
# restricted sweep prevents an accidental change from being presented as a
# controller redesign; ±7 remains an extrapolation test beyond parts of the
# observed training-command support.
_ALLOWED_INPUT_CAPS = (6.0, 6.5, 7.0)
INPUT_COMMAND_BOUND_STANDARDIZED = float(
    os.environ.get("COPYBH_INPUT_CAP", "7.0")
)
if INPUT_COMMAND_BOUND_STANDARDIZED not in _ALLOWED_INPUT_CAPS:
    raise ValueError("COPYBH_INPUT_CAP must be 6.0, 6.5, or 7.0")
...
# Multi-face stress path: center → face → on-face diagonal slide → center.
# Pressures sit above the H=18 chance mean bounds (≈2.94/3.15/3.58) and
# inside the source hard box so risk constraints activate while remaining
# geometrically drawable. Reference-only; controller/covariance unchanged.
FACE_PETAL_PRESSURES = (3.40, 3.40, 3.57)
FACE_PETAL_SLIDE = 0.75
FACE_PETAL_DEFAULT_STEPS = 18000
# Default: large-arc axial near-hard envelope (v3.9) — chance-QP friendly.
DEFAULT_REFERENCE_MODE = "growing-smooth-square-helix"
WIND_SEED = 7007
INNOVATION_SEED = 7
OUTPUT_STEM = "copyBH_pelican_u_cap_ablation"
...
def xy_face_dwell_reference(steps, x_pressure=3.57, y_pressure=3.57):
    """Center-started x/y face dwells for the input-cap ablation only.

    The schedule is center→x face→center→y face→center. Each face is held
    for 20% of the run; quintic transitions have zero endpoint velocity and
    acceleration. This changes only the exogenous reference, not the frozen
    model, controller, covariance, chance tightening, or disturbance model.
    """
    steps = int(steps)
    x_pressure = float(x_pressure)
    y_pressure = float(y_pressure)
    if steps < 40:
        raise ValueError("steps must be at least 40")
    if x_pressure <= 0.0 or y_pressure <= 0.0:
        raise ValueError("x_pressure and y_pressure must be positive")

    knot_times = np.array([0.0, 0.15, 0.35, 0.50, 0.65, 0.85, 1.0])
    anchors = np.array([
        [0.0, 0.0, 0.0],
        [x_pressure, 0.0, 0.0],
        [x_pressure, 0.0, 0.0],
    ...
    if reference.shape != (steps, 3):
        raise RuntimeError(
            f"x/y face dwell construction returned {reference.shape}, "
            f"expected {(steps, 3)}"
        )
    return reference


def _quintic_path_from_anchors(anchors, steps):
    """Equal-time quintic blend through waypoints (C1-ish, zero end accel)."""
    anchors = np.asarray(anchors, dtype=float).reshape(-1, 3)
    steps = int(steps)
    if anchors.shape[0] < 2:
        raise ValueError("at least two anchors are required")
    if steps < anchors.shape[0]:
        raise ValueError("steps must be at least the number of anchors")
    knot_times = np.linspace(0.0, 1.0, anchors.shape[0], endpoint=True)
    timeline = np.linspace(0.0, 1.0, steps, endpoint=True)
    segment = np.minimum(
        np.searchsorted(knot_times[1:], timeline, side="right"),
        anchors.shape[0] - 2,
    ...
    left = knot_times[segment]
    right = knot_times[segment + 1]
    local = (timeline - left) / np.maximum(right - left, 1e-15)
    blend = local ** 3 * (10.0 - 15.0 * local + 6.0 * local ** 2)
    reference = (
        (1.0 - blend[:, None]) * anchors[segment]
        + blend[:, None] * anchors[segment + 1]
    ...
    if reference.shape != (steps, 3):
        raise RuntimeError(
            f"anchor path construction returned {reference.shape}, "
            f"expected {(steps, 3)}"
        )
    return reference


def face_petal_stress_reference(
        steps=FACE_PETAL_DEFAULT_STEPS,
        pressures=FACE_PETAL_PRESSURES,
        slide=FACE_PETAL_SLIDE):
    """Six-face petal stress: center → face → diagonal on-face slide → center.

    Face order: +x, −x, +y, −y, +z, −z. Side-face petals climb a modest
    height ladder while sliding left/right on the face; z-face petals do a
    small horizontal diamond on the face. Targets sit above the terminal
    chance mean bounds so SMPC risk constraints can activate, while remaining
    strictly inside the source hard box. Reference trajectory only.
    """
    steps = int(steps)
    pressures = np.asarray(pressures, dtype=float).reshape(3)
    slide = float(slide)
    if steps < 120:
        raise ValueError("face-petal-stress needs at least 120 samples")
    if np.any(pressures <= 0.0):
        raise ValueError("face-petal pressures must be positive")
    if slide < 0.0:
        raise ValueError("slide amplitude must be non-negative")
    if slide >= float(np.min(pressures[:2])):
        raise ValueError("slide must be smaller than x/y face pressures")

    px, py, pz = (float(pressures[0]), float(pressures[1]), float(pressures[2]))
    # Side-face height ladder keeps petals visually separated in 3D.
    z_levels = [0.0, 0.45, 0.90, 1.35, 1.80]
    anchors = []

    def add_side_petal(axis, sign, z0, z1):
        """axis: 0=x face, 1=y face; sign: ±1."""
        p = px if axis == 0 else py
        face0 = np.zeros(3)
        face0[axis] = sign * p
        face0[2] = z0
        # On-face free axis is the other horizontal axis.
        free = 1 - axis
        right = face0.copy()
        right[free] = slide
        right[2] = 0.5 * (z0 + z1)
        ...
        anchors.extend([center0, face0, right, left, settle, center1])

    def add_z_petal(sign, z_from):
        face = np.array([0.0, 0.0, sign * pz])
        c0 = np.array([0.0, 0.0, z_from])
        # Horizontal diamond on the z face (clear multi-axis motion).
        a = np.array([slide, 0.0, sign * pz])
        b = np.array([0.0, slide, sign * pz])
        c = np.array([-slide, 0.0, sign * pz])
        d = np.array([0.0, -slide, sign * pz])
        # Return to midline at the face height, then a short drop/rise toward 0
        # only after the last petal (handled by final center).
        c1 = np.array([0.0, 0.0, sign * pz])
        anchors.extend([c0, face, a, b, c, d, face, c1])

    # +x, -x, +y, -y
    add_side_petal(0, +1.0, z_levels[0], z_levels[1])
    add_side_petal(0, -1.0, z_levels[1], z_levels[2])
    add_side_petal(1, +1.0, z_levels[2], z_levels[3])
    add_side_petal(1, -1.0, z_levels[3], z_levels[4])
    # +z, return through origin, then -z; finish at origin for a closed path.
    # (Direct +pz → -pz would be one huge vertical jump under equal-time knots.)
    add_z_petal(+1.0, z_levels[4])
    anchors.append(np.array([0.0, 0.0, 0.0]))
    add_z_petal(-1.0, 0.0)
    anchors.append(np.array([0.0, 0.0, 0.0]))

    # Drop consecutive duplicate anchors (shared centers between petals).
    cleaned = [np.asarray(anchors[0], dtype=float)]
    for point in anchors[1:]:
        point = np.asarray(point, dtype=float)
        if np.linalg.norm(point - cleaned[-1]) > 1e-12:
            cleaned.append(point)
    reference = _quintic_path_from_anchors(cleaned, steps)
    return reference
...
def equilibrium_initial_condition(model, E, target):
    ...
    if basis.shape[1] == 1:
        ...
        for value, slope in zip(solution[n:, 0], direction):
            if abs(slope) < 1e-14:
                if abs(value) > INPUT_COMMAND_BOUND_STANDARDIZED + 1e-9:
                    raise ValueError("No bounded-input equilibrium")
                continue
            interval = sorted((
                (-INPUT_COMMAND_BOUND_STANDARDIZED - value) / slope,
                (INPUT_COMMAND_BOUND_STANDARDIZED - value) / slope,
            ))
    ...
    if np.any(np.abs(control) > INPUT_COMMAND_BOUND_STANDARDIZED + 1e-8):
...
def simulate_controller(model, E, hard_bound, reference, residual_innovation,
    ...
    saturation = (
        np.any(
            np.abs(control_history) >= INPUT_COMMAND_BOUND_STANDARDIZED - 1e-4,
            axis=1,
        )
        if completed_steps else np.zeros(0, dtype=bool)
    )
...
def plot_controller_diagnostics(path, controller_name, result, reference,
    ...
    input_axis.axhline(
        INPUT_COMMAND_BOUND_STANDARDIZED, color="#333333", linewidth=0.8,
    )
    input_axis.axhline(
        -INPUT_COMMAND_BOUND_STANDARDIZED, color="#333333", linewidth=0.8,
    )
    ...
    figure.suptitle(
        f"copyBH u-cap={INPUT_COMMAND_BOUND_STANDARDIZED:g} {controller_name}: "
        "Pelican model-in-the-loop, "
...
def plot_noise_actual_vs_estimated(
    ...
    figure.suptitle(
        "copyBH actual noise vs estimated noise  |  time series  |  "
...
def render_forward_view_gif(
        ...
        fps=FORWARD_GIF_FPS,
        title="copyBH SMPC — forward chase view"):
...
def run_experiment(steps=None, sigma_wind_mps=SIGMA_WIND_MPS,
    ...
    if steps is None:
        steps = (
            ...
            else FACE_PETAL_DEFAULT_STEPS
            if reference_mode == "face-petal-stress"
            else DEFAULT_STEPS
        )
    steps = int(steps)
    ...
    if pressures is None:
        ...
        elif reference_mode == "xy-face-dwell":
            pressures = np.array([3.57, 3.57, 0.0])
        elif reference_mode == "face-petal-stress":
            pressures = FACE_PETAL_PRESSURES
        else:
    ...
    if reference_mode == "smooth-square-helix":
        ...
        if not dc_feasibility["feasible"]:
            raise ValueError(
                f"{reference_mode} end pressures are DC-infeasible under the implemented "
                f"±{INPUT_COMMAND_BOUND_STANDARDIZED:g} inputs: "
        ...
        # Sustained paths must satisfy the selected standardized-command cap.
    ...
    if reference_mode == "smooth-square-helix":
        ...
        reference = xy_face_dwell_reference(
            steps,
            x_pressure=float(pressures[0]),
            y_pressure=float(pressures[1]),
        )
        reference_path_desc = (
            "center → x-face dwell (20%) → center → y-face dwell (20%) → "
            f"center at targets {(float(pressures[0]), float(pressures[1]))}"
        )
    elif reference_mode == "face-petal-stress":
        reference = face_petal_stress_reference(
            steps=steps,
            pressures=pressures,
            slide=FACE_PETAL_SLIDE,
        )
        reference_path_desc = (
            "six-face petal stress: center→face→on-face diagonal slide→center "
            "for +x/−x/+y/−y/+z/−z; "
            f"pressures={(float(pressures[0]), float(pressures[1]), float(pressures[2]))}, "
            f"slide={FACE_PETAL_SLIDE:g}; targets above chance mean bounds, "
            f"inside hard={hard_bound:g}"
        )
    else:
    ...
    if smpc["fallback_count"] or deterministic_mpc["fallback_count"]:
        raise RuntimeError("Fallback is forbidden in copyBH")
    ...
    if make_forward_gif and int(smpc["completed_steps"]) >= 200:
        ...
        gif_meta = render_matlab_style_playback_gif(
            ...
            dt=SAMPLE_TIME_SECONDS,
            title=(
                f"copyBH u-cap={INPUT_COMMAND_BOUND_STANDARDIZED:g} SMPC playback | "
                f"{reference_mode} "
    ...
    summary = {
        ...
        "parent_experiment": "copyBG_pelican_physical_wind_smpc",
        "ablation_variable": "INPUT_COMMAND_BOUND_STANDARDIZED",
        ...
        "Ru": RU,
        "alpha_joint": ALPHA_JOINT,
        "input_bound_normalized": [
            -INPUT_COMMAND_BOUND_STANDARDIZED,
            INPUT_COMMAND_BOUND_STANDARDIZED,
        ],
        ...
        "smpc_vs_mpc": {
            ...
            "note": (
                "Input-cap-only ablation: the selected normalized "
                f"±{INPUT_COMMAND_BOUND_STANDARDIZED:g} experiment cap is not "
                "a claimed Pelican hardware rating. Identification, Q/Ru, "
                "horizon, chance tightening, QP structure, reference, and "
                "disturbance seeds remain fixed across the sweep."
            ),
        },
        "interpretation": (
            "The deterministic MPC trace ends at its first non-optimal QP; "
            "no fallback or post-failure trajectory is synthesized. "
            "This copy changes only the standardized motor-command box. "
            "Any unchanged x/y ceiling despite a larger cap is evidence that "
            "the frozen chance mean bound, rather than the input box, is the "
            "active limitation in this experiment."
        ),
...
def _main():
    parser = argparse.ArgumentParser(
        description=(
            "Run copyBH Pelican input-cap ablation with the frozen copyBG "
            "controller, model, chance tightening, and wind."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "face-petal-stress (center→six faces with on-face slides), "
            "boundary-tour (single-axis SMPC legs), "
            "xy-face-dwell, smooth-square-helix, circle-helix, "
