...
% ===== Zfix (滑动窗口+更新Pbar+Z2非对称soft+SMPC) =====
...
for k=1:T_cl
    ...
    if mod(k,Nr)==0 && k>0 && (k-tl)>=Nr
        % ★ 滑动窗口: 只用最近200步数据
        yc_in = yc(:, end-min(size(yc,2),199):end);
        uc_in = uc(:, end-min(size(uc,2),199):end);
...
print('copyZfix_fig','-dpng','-r150');
fprintf('figure saved: copyZfix_fig.png\n');