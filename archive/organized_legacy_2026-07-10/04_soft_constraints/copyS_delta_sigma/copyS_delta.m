%% 副本 S — 增量σ软约束 (Δσ = sig(k)-sig(k-1) 驱动)
...
for k=1:T_cl
    ...
    % ★ 增量σ软约束: Δσ = sig(k) - sig(k-1), 噪声增长时收紧
    dsig = sig - sig_prev;
    soft = max(0, dsig);  % 只在σ增长时施加
    softZ(k)=soft; zk=xk;
    sig_prev = sig;  % 更新
    Nv=12;
    H=zeros(Nv*m); f=zeros(Nv*m,1); ya=Pk*zk;
...
save('C vs DeltaSig_data.mat','-struct','sv');
fprintf('done: C vs DeltaSig_data.mat\n');
...
sgtitle(sprintf('C vs DeltaSig: C(e=%.3f,viol=%d) vs Zfix(e=%.3f,viol=%d)',eC,vC,eZ,vZ),'FontSize',15,'FontWeight','bold');
print('C vs DeltaSig_fig','-dpng','-r150');
fprintf('figure saved: C vs DeltaSig_fig.png\n');