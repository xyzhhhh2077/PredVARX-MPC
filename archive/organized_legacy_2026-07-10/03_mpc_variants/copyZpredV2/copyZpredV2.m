%% 副本 ZpredV2 — PredVAR原文对齐: smooth噪声, ℓ=n=6, T_off=5000
clear; clc; addpath(fileparts(mfilename('fullpath')));

n=6; m=3; p=15; ell=6;  % ★ ℓ=n=6, 消除信号泄漏
N_pred=30; T_cl=10000; T_off=5000;  % ★ 增大5×
...
% ★ 平滑噪声: 正弦调制, 从sw0*(1-0.5)到sw0*(1+0.5)
T_total=T_off+T_cl;
sw_seq=sw0*(1+0.5*sin(2*pi*(1:T_total)/3000));  % 周期3000步
se_seq=se0*(1+0.5*sin(2*pi*(1:T_total)/3000+pi/3));  % 相位偏移
...
Q=zeros(p); Q(1:min(ell,p),1:min(ell,p))=eye(min(ell,p)); Rw=2*eye(m);
nz=ell; Ex=eye(ell);

xo=zeros(n,T_off+1); yo=zeros(p,T_off); uo=10*randn(m,T_off);
for k=1:T_off
    yo(:,k)=C*xo(:,k)+se_seq(k)*randn(p,1);
    xo(:,k+1)=A*xo(:,k)+B*uo(:,k)+sw_seq(k)*randn(n,1);
end
...
fprintf('ℓ=%d T_off=%d 平滑噪声...\n',ell,T_off);

% ===== ZV2: PredVAR对齐 + Z2 soft =====
yZ=zeros(p,T_cl); uZ=zeros(m,T_cl);
sZ_eps=zeros(1,T_cl); sZ_xi=zeros(1,T_cl); costZ=zeros(1,T_cl); softZ=zeros(1,T_cl);
...
for k=1:T_cl
    k_abs=k+T_off;
    sw_smooth=sw_seq(k_abs); se_smooth=se_seq(k_abs);
    yk=C*xt+se_smooth*randn(p,1); yZ(:,k)=yk;
    xk=Rk'*yk;

    % PredVAR对齐: e_k = P ε_k + P_⊥ ξ_k
    ...
    sZ_xi(k)=sqrt(trace(Sxi)/max(p-ell,1));
    ...
    for j=1:Nv
        ...
        sft=zeros(p,1); sft(1:min(ell,p))=soft;
    ...
    xt=A*xt+B*uk+sw_smooth*randn(n,1);
...
fprintf('ZV2=%.3f(%d)  σ_eps=%.4f σ_xi=%.4f (true≈0.1)\n',...
    eZ,vZ,mean(sZ_eps(500:end)),mean(sZ_xi(500:end)));

sv.yZ=yZ; sv.uZ=uZ; sv.sZ_eps=sZ_eps; sv.sZ_xi=sZ_xi;
sv.sw_seq=sw_seq(T_off+1:end); sv.se_seq=se_seq(T_off+1:end);
sv.costZ=costZ; sv.softZ=softZ; sv.Rf=Rf; sv.y_max=y_max;
sv.eZ=eZ; sv.vZ=vZ; sv.T_cl=T_cl; sv.ell=ell; sv.T_off=T_off;
save('copyZpredV2_data.mat','-struct','sv');
fprintf('done: copyZpredV2_data.mat\n');