%% 副本 PredPCA — PCA降维+VARX回归 → MPC vs SMPC (硬约束)
clear; clc;

n=6; m=3; p=15; ell=4; N_pred=30; T_cl=10000; T_off=200;
...
B_r=randn(n,m); C_r=randn(p,n); [C_r,~]=qr(C_r,0);
...
for k=1:T_off, yo(:,k)=C_r*xo(:,k)+se0*randn(p,1); xo(:,k+1)=A*xo(:,k)+B_r*uo(:,k)+sw0*randn(n,1); end

% ── PCA降维 (替代 PredVAR IVR) ──
N=T_off-L_u; Ph=zeros(N,m*L_u);
for i=1:N, t=L_u+i; ul=[]; for lag=1:L_u, ul=[ul;uo(:,t-lag)]; end; Ph(i,:)=ul'; end
Yr=yo(:,L_u+1:L_u+N); Cu=(Ph'*Ph+1e-6*eye(m*L_u))\(Ph'*Yr');
yr=Yr-(Ph*Cu)'; yr=yr-mean(yr,2);  % 剥离u,中心化

[Uy,~,~]=svd(yr*yr'/N,'econ');
P0=Uy(:,1:ell); R0=(P0'*P0)\P0';
xl=R0*yr; xn=xl(:,2:end); xc=xl(:,1:end-1); ur=uo(:,L_u+1:L_u+N-1)-mean(uo(:,L_u+1:L_u+N-1),2);
Th=([xc;ur]*[xc;ur]'+1e-6*eye(ell+m))\([xc;ur]*xn');
A0=Th(1:ell,:)'; B0=Th(ell+1:end,:)';
Pbr0=null(P0'); G0=eye(ell); Se0=sw0^2*eye(ell);

fprintf('PCA: P0=%dx%d A0=%dx%d B0=%dx%d\n',size(P0,1),size(P0,2),size(A0,1),size(A0,2),size(B0,1),size(B0,2));

Ac=A0; Mc=cell(1,N_pred); Nc=cell(1,N_pred);
...
Ak=Ac; Pk=P0; Rk=R0; Akh=A0; Bk=B0; PbrS=Pbr0; Mk=Mc; Nk=Nc;

for k=1:T_cl
    yk=C_r*xt+se0*randn(p,1); yC(:,k)=yk; xk=Rk*yk;
...
for k=1:T_cl
    yk=C_r*xt+se0*randn(p,1); yS(:,k)=yk; xk=Rk*yk;
    ...
    % ★ SMPC硬约束: y_max ≥ μ₀(j)  (无βσ)
    ncc=2*ell*N_pred; A_cc=zeros(ncc,Nv*m); b_cc=zeros(ncc,1); ri=0;
    for jj=1:min(Nv,N_pred)
        Nfj=Nk{jj}(:,1:Nv*m); mu0j=Mk{jj}*xk;
        for ch=1:ell
            ri=ri+1; A_cc(ri,:)= Nfj(ch,:); b_cc(ri)=y_max-mu0j(ch);       % 硬上界
        ri=ri+1; A_cc(ri,:)=-Nfj(ch,:); b_cc(ri)=mu0j(ch)+3.0;
        end          % 硬下界
    end
...
fprintf('sCe=%.3f sSe=%.3f\\n',mean(sCe(500:end)),mean(sSe(500:end)));
...
ylabel('y_2');title(sprintf('PCA PredVAR: C=%.3f(%d) SM=%.3f(%d) bnd=%d',eC,vC,eS,vS,sum(bnd)));
...
sgtitle('PCA PredVARX + SMPC 硬约束(无noise)');
saveas(gcf,'predpca_smpc.png');
fprintf('图: predpca_smpc.png\\n');
...
save('predpca_data.mat','-struct','sv');
fprintf('done\\n');