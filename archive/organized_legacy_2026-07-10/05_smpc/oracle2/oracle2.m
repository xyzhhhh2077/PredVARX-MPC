...
for k=1:T_cl
    ...
    for jj=1:min(Nv,N_pred)
        ...
        for ch=1:ell
            ...
            ri=ri+1; A_cc(ri,:)= Nfj(ch,:); b_cc(ri)=y_max-mu0j(ch)      % 硬约束(无noise);
            ri=ri+1; A_cc(ri,:)=-Nfj(ch,:); b_cc(ri)=mu0j(ch)+3.0          % 硬约束(无noise);
        end
    end
...
ylabel('y_2');title(sprintf('Oracle2(hard): C=%.3f(%d) SM=%.3f(%d) bnd=%d',eC,vC,eS,vS,sum(bnd)));
...
sgtitle('Oracle2: 真A,B,C已知 → MPC vs SMPC');
saveas(gcf,'oracle2_hard.png');
fprintf('图: oracle2_hard.png\\n');
...
save('oracle2_data.mat','-struct','sv');
fprintf('done\\n');