...
y_max=3.2; rv=[1,3,0.2,2.5,0.5]; nf=[1,1.5,0.8,2,0.5,1.2,3,0.6,1,1.8]; ni=1000;
...
for k=1:T_cl
    ns=min(floor((k-1)/ni)+1,length(nf)); sw_k=sw0*nf(ns); se_k=se0*nf(ns);
    yk=C*xt+se_k*randn(p,1); yC(:,k)=yk; xk=Rk*yk;
    % Dinkla Σ_ε
...
for k=1:T_cl
    ns=min(floor((k-1)/ni)+1,length(nf)); sw_k=sw0*nf(ns); se_k=se0*nf(ns);
    yk=C*xt+se_k*randn(p,1); yS(:,k)=yk; xk=Rk*yk;
    ...
    for jj=1:min(Nv,N_pred)
        ...
        for ch=1:ell
            % tv removed (hard constraint)
            ri=ri+1; A_cc(ri,:)= Nfj(ch,:); b_cc(ri)=y_max-mu0j(ch);
...
ylabel('y_2');title(sprintf('Oracle3(时变噪声): C=%.3f(%d) SM=%.3f(%d) bnd=%d',eC,vC,eS,vS,sum(bnd)));
...
legend('C','SM2-hard','true','Location','best');ylabel('sigma_eps');
...
for ss=2000:2000:T_cl,xline(ss,'Color',[.85 .85 .85]);end
legend('C','SM2-hard','Location','best');ylabel('J_k (log)');
...
sgtitle('Oracle3(时变噪声): 真A,B,C已知 → MPC vs SMPC');
saveas(gcf,'oracle3_tvar.png');
fprintf('图: oracle3_tvar.png\\n');
...
save('oracle3_data.mat','-struct','sv');
fprintf('done\\n');