%% PredVARX-MPC v10 副本 - Route C-Delta 三段阶跃 + 增量加速组合 (副本 E: 极短 N)
%
%  ★ 副本数学公式与 routeC_delta_3step 完全一致, 仅修改 MPC 在线求解:
%    [副本 E = P1 + P3 加强版]
%    [P1] QP warm-start + Σ 协方差增量 (同 C)
%    [P3 加强] 二阶段 N_curr:
%      - k < 400: N = N_short = 3 (极度激进短 N)
%      - k ≥ 400: N = N_max = 30
%
%  目的: 看极度短 N 在前段 (k<400) 是否仍能保持跟踪 (Δ-tracking 数学应使其不敏感)
%  代价: 段 1 (ref=1.0) 短暂 N=3 可能引入精度损失, 看量化影响
%
%  ★ 副本数学公式与 routeC_delta_3step 完全一致, 仅修改 MPC 在线求解:
%    [副本 C = P1 + P3 组合]
...
for k = 1:T_cl
    ...
    % ★ P3: 自适应预测步数
    if k < 400
        N_curr = N_short;       % 早期: 极短 N (3)
    else
        N_curr = N_max_pred;    % 中后期: 全 N
    end
    N_history(k) = N_curr;
