%% PredVARX-MPC v10 副本 I — Dinkla 2026 顺序单步 + 累积辨识
%  在线子空间辨识: Dinkla 顺序单步预测 + 累积 ZtZ/YtZ + 窗口 Σ_ε
%
%  ★ 副本 I = P1 + P3 + Dinkla 顺序单步 + 累积辨识:
%    [P1] QP warm-start + Σ 增量
%    [P3] 自适应预测步数 N(k)
%    [Dinkla 顺序单步] 多步预测分解为单步迭代, 避免 A_cl^j 预计算
%    [Dinkla Σ_ε] 窗口基协方差 (替换遗忘因子)
%    [累积 ZtZ/YtZ] 新数据追加不丢弃, 周期性完整重辨识
%      - k < 200:        N = N_short = 5
...
% 在线协方差初始化 (§3.3 文档)
% Dinkla 2026: 窗口基 Σ_ε (替换遗忘因子)
Sigma_eps_k = Sigma_eps_hat;
Sigma_ebar_k = Sigma_ebar_hat;
N_win_dinkla = 50;                        % Dinkla 窗口长度
e_buffer = zeros(ell, N_win_dinkla);       % 创新缓冲
e_buffer_count = 0;
...
reid_time_total = 0;    % ★ 副本 I: 累计在线重辨识时间 (s)
opt_qp = optimset('Display', 'off', 'LargeScale', 'off');

% ★ 副本 I: 累积数据缓冲 (追加不丢弃) + 周期性重辨识
N_reid = 500;                             % 重辨识周期 (拍数)
y_cum = y_off;                             % 累积 y 数据 (离线 + 在线)
u_cum = u_off;                             % 累积 u 数据
...
for k = 1:T_cl
    ...
    % --- Algorithm 2 步骤 3: Dinkla 2026 窗口基 Σ_ε 更新 (§3.3) ---
    % 替换遗忘因子: 用固定窗口计算创新协方差
    if k >= 2
        e_k = x_hat_k - A_hat * x_hat_hist(:, k-1) - B_hat * u_hist(:, k-1);
        e_buf_idx = mod(k-1, N_win_dinkla) + 1;
        e_buffer(:, e_buf_idx) = e_k;
        e_buffer_count = min(e_buffer_count + 1, N_win_dinkla);
        if e_buffer_count >= N_win_dinkla
            Sigma_eps_k = (e_buffer * e_buffer') / N_win_dinkla;
    ...
    % --- 副本 I: 累积数据 (追加到离线数据后, 不丢弃) ---
    y_cum = [y_cum, y_k];
    u_cum = [u_cum, u_k];

    % --- 副本 I: 周期性用累积数据重辨识 ---
    if mod(k, N_reid) == 0 && k > 0 && (k - t_reid_last) >= N_reid
        tic_reid = tic;
        P_hat_old = P_hat;
        A_hat_old = A_hat;
        [A_hat, B_hat, P_hat, Pbar_hat, R_hat, Rbar_hat, G_aug_hat, Sigma_eps_hat, Sigma_ebar_hat, F_aug_hat, H_aug_hat, ~] = predvarx_identify(y_cum, u_cum, ell, beta_u, L_u, A_true, B_true, C_true, n, m, p);
        A_cl_hat = F_aug_hat;
        % 更新 M_cl, N_cl (Dinkla 顺序单步逻辑, 用新 P̂, A_cl)
        for j = 1:N_pred
            M_cl{j} = P_hat * E_x' * A_cl_hat^j;
            for i = 0:j-1
                N_cl{j}(:, i*m+1:(i+1)*m) = P_hat * E_x' * A_cl_hat^(j-1-i) * H_aug_hat;
        ...
        eig_shift = norm(eig(A_hat_old) - eig(A_hat));
        t_reid_last = k;
        reid_time_total = reid_time_total + toc(tic_reid);
        fprintf('  [副本 I] k=%d 累积辨识 (N=%d): ||P̂-P̂_old||=%.3f, eig移=%.3f (累计 %.3fs)\n', k, size(y_cum,2), sub_drift, eig_shift, reid_time_total);
    end
