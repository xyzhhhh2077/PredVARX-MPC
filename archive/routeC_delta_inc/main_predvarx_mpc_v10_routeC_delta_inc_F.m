%% PredVARX-MPC v10 副本 - Route C-Delta 三段阶跃 + 增量加速组合 (副本 F: 误差自适应)
%
%  ★ 副本数学公式与 routeC_delta_3step 完全一致, 仅修改 MPC 在线求解:
%    [副本 F = P1 + P3-强化]
%    [P1] QP warm-start + Σ 协方差增量 (同 C)
%    [P3-强化] 误差自适应 N_curr:
%      - 初段 k < N_pre:   N = N_short = 5   (烧入)
%      - 跟踪误差大 (|y_k - r| > ε): N = N_max = 30  (需要远视)
%      - 跟踪误差小:        N = N_short = 5   (近距已足够)
%
%  假设: Δ-tracking 数学使得 ref 阶跃时误差突增, 之后指数衰减
%        误差自适应 N 应该在过渡段用 N=30 (看更远), 稳态段用 N=5 (省算力)
%  风险: 误差信号可能噪声, 抖动 N 不一定省
%
%  ★ 副本数学公式与 routeC_delta_3step 完全一致, 仅修改 MPC 在线求解:
%    [副本 C = P1 + P3 组合]
...
for k = 1:T_cl
    ...
    % ★ P3: 自适应预测步数
    % ★ P3 强化: 误差自适应 N_curr
    y_max_idx = max(min(k, T_cl), 1);  % ref 索引
    if k < 100
        N_curr = N_short;       % 烧入期
    else
        % 跟踪误差: ||y_k - r_{k}|| (使用前 ℓ 通道)
        r_curr = y_ref_full(1:ell, y_max_idx);
        e_norm = norm(y_true(1:ell, k) - r_curr);
        % 切换阈值: ε = 0.3 (ref 量级 ~1)
        if e_norm > 0.3
            N_curr = N_max_pred;  % 大误差: 看更远
        else
            N_curr = N_short;     % 小误差: 近距已足够
        end
    end
    N_history(k) = N_curr;
