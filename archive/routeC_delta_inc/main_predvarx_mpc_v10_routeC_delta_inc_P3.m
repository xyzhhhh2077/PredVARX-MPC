%% PredVARX-MPC v10 副本 - Route C-Delta 三段阶跃 + 自适应预测步数 P3
%
%  ★ 副本数学公式与 routeC_delta_3step 完全一致, 仅修改 MPC 在线求解:
%    [P3] 自适应预测步数 N(k):
%      - 早期 (k < burn_in): N = N_short = 5  (烧入期 + 初期, 用短 N 快速迭代)
%      - 中期 (k 中段):        N = N_mid  = 15 (收尾稳态后回短 N)
%      - 后期 (k 接近终点):   N = N_long = 30 (稳态下需要长 N 求高精度)
%      - 切换判据: 跟踪误差 e(k) = ||y_k - r_k||
%
%  数学严格性: N(k) 自适应不影响 Δ-tracking 数学, 仅 QP 维度变化
%  三段阶跃同 P1
%
