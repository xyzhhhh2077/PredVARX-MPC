%% PredVARX-MPC v10 副本 - Route C-Delta 三段阶跃 + 增量加速 P1
%
%  ★ 副本数学公式与 routeC_delta_3step 完全一致, 仅修改 MPC 在线求解:
%    [P1] QP warm-start: 用上一步最优解 c_opt_{k-1} 平移一格作为初值
%       + 协方差增量: Σ_z^{cl}(j,k+1) 初值取 Σ_z^{cl}(j,k), 只做 1 次 Lyapunov 校正
%
%  目的: 量化解 Dinkla 2026 增量迭代思路在你的 v10 Δ-tracking 上的加速比
%    - 副本数学公式 0 改动 (与主版完全一致)
%    - 改动只在 §4.3 Algorithm 2 在线循环的第 5/6/7 步
%
%  三段阶跃参照 routeC_delta_3step (ref = 1.0 → 2.0 → 0.5)
...
% ★ P1 增量加速: warm-start 缓存 + timing
c_opt_prev = zeros(N_pred*m, 1);             % 上一步最优解 (供下一拍 warm-start)
Sigma_z_prev_k_outer = zeros(nz, nz);        % k-1 拍的最末 Σ_z (供下一拍 Lyapunov 重用)
Sigma_z_history = zeros(nz, nz, N_pred);     % 存 Σ_z^{cl}(j=1:N, k) 给下拍作初值 (增量 Δ)
warm_start_used = zeros(1, T_cl);            % 记录哪些拍用了 warm-start
qp_time_total = 0;                            % 累计 QP 求解时间 (s)
qp_iter_total = 0;                            % 累计 QP 内迭代次数
...
for k = 1:T_cl
    ...
    % ★ P1 增量: 用上一拍的 Σ_z^{cl}(j, k-1) 作 j=1 的初值 (shift 加速)
    %        Σ_z^{cl}(j, k) 与 Σ_z^{cl}(j-1, k-1) 在采样间隔间差异极小 (ΔΣ ≈ (A_cl - I)·Σ + Q_aug)
    %        1 次 Lyapunov 重用 ≈ 0 次全递推 (在 Σ_eps 变化慢的场景下)
    tic_cov = tic;
    if k > 1
        % 上一拍 Σ_z^{cl}(j, k-1) → 这一拍 Σ_z^{cl}(j+1, k) 的 1 次 update
    ...
    for j = 1:N_pred
        ...
        if j == 1
            % Δy_1 = (M_1 z_k + N_1 c) - y_k_actual
            % r_1 - y_k_actual = Δr_1
            % (y_pred_1 - r_1) = (M_1 z_k - r_1) + N_1 c
            %                  = (y_k_actual + (M_1 z_k - y_k_actual) + Δr_1 - Δr_1 - r_1 + y_k_actual) + N_1 c
            % Δr_1 = r_1 - y_k_actual
            delta_r = r_j - y_k_actual;  % p × 1
            M_delta = M_cl{j};             % Δy_1 = M_1 z_k - y_k + N_1 c
            N_delta = N_cl{j};
            f_const = -y_k_actual;
        else
            % Δy_j = y_{k+j|k} - y_{k+j-1|k} = (M_j - M_{j-1}) z_k + (N_j - N_{j-1}) c
            r_prev = y_ref_full(:, min(k+j-1, T_cl));
            delta_r = r_j - r_prev;       % Δr_j = r_j - r_{j-1}
    ...
    warm_start_used(k) = 1;     % 标记本拍用了 warm-start
...
% ★ P1: dump 增量加速统计
fprintf('\n=== P1 增量加速统计 ===\n');
fprintf('QP 累计求解时间: %.4f s (mean %.6f s/step)\n', ...
    qp_time_total, qp_time_total/T_cl);
fprintf('Σ 协方差累计递推时间: %.4f s (mean %.6f s/step)\n', ...
    cov_time_total, cov_time_total/T_cl);
fprintf('Warm-start 命中拍数: %d/%d (%.1f%%)\n', ...
    sum(warm_start_used), T_cl, 100*sum(warm_start_used)/T_cl);
