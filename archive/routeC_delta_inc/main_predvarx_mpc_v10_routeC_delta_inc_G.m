%% PredVARX-MPC v10 副本 - Route C-Delta 三段阶跃 + 增量加速组合 (副本 G: Dinkla 顺序单步 + IV)
%
%  ★ 副本数学公式与 routeC_delta_3step 完全一致, 仅修改 MPC 在线求解:
%    [副本 C = P1 + P3 组合]
%    [P1] QP warm-start: 用 c_opt_{k-1} 平移一格作为下一拍 quadprog 初始点
%       + 协方差时间步增量: Σ_z^{cl}(j,k+1) 初值取 Σ_z^{cl}(N,k), 1 次 Lyapunov 重用
%    [P3] 自适应预测步数 N(k):
%      - k < 200:        N = N_short = 5
...
%    [Dinkla 顺序单步 + IV] 替代遗忘方法更新 Σ_ε:
%       - 单步 LS: L̂_1 = Y Z^T (Z Z^T)^\dagger
%       - IV 修正: L̂_1^IV = Y W^T (Z W^\dagger)^\dagger (W = 闭环下与 e_k 不相关)
%       - Σ̂_ε 更新: ê_k ê_k^T, 无遗忘
%       - 数学严格性: a.s. 一致性 (Dinkla 2026 Theorem)
...
% lambda_forget = 0.95;  % 副本 C 用的遗忘因子 (副本 G 不再用)
% ★ 副本 G: Dinkla 顺序单步 LS 的滑动窗口长度
N_iv = 30;                   % IV 估计窗口 (与 N_pred 同)
...
for k = 1:T_cl
    ...
    if k >= 2
        ...
        % ★ 副本 G: Dinkla 2026 顺序单步 + IV 修正 (替代遗忘方法)
        %   步骤 1: 单步 LS 估 L̂_1 (用滑动窗口 N_iv 拍)
        %   步骤 2: IV 修正: 工具变量 W_k = [u_k; y_{k-1}; u_{k-1}]
        %   步骤 3: L̂_1^IV = Y W^T (Z W^\dagger)^\dagger
        %   步骤 4: Σ̂_ε(k) = ê_k ê_k^T (无遗忘)
        if k > N_iv
            iv_win = k - N_iv + 1 : k;
            Y_iv = y_true(:, iv_win);                    % p × N_iv
            u_iv = u_hist(:, iv_win);                    % m × N_iv
            Z_iv = [u_iv; y_true(:, iv_win-1); u_hist(:, iv_win-1)];  % (m+p+m) × N_iv
                        W_iv = [u_iv; y_true(:, iv_win-1); u_hist(:, iv_win-1)];  % IV (闭环下与 e_k 不相关)
                        % 单步 LS: L̂_1 = Y Z^T (Z Z^T)^\\dagger  → p × (m+p+m)
                        L1_ls = Y_iv * Z_iv' / (Z_iv * Z_iv' + 1e-8*eye(m+p+m));
                        % IV 修正: L̂_1^IV = Y W^T (Z W^\\dagger)^\\dagger  → p × (m+p+m)
                        P_W = pinv(W_iv');                  % (m+p+m) × N_iv
                        L1_iv = Y_iv * W_iv' * pinv(Z_iv * P_W);
                        % 烧入后用 IV, 之前用 LS (避免矩阵接近奇异)
                        if k > 2 * N_iv
                            L1_use = L1_iv;
                        else
                            L1_use = L1_ls;
                        end
            % 单步预测: ê_k = y_k - L1_use [u_k; y_{k-1}; u_{k-1}]
            z_k_now = [u_hist(:, k); y_true(:, k-1); u_hist(:, k-1)];
            e_k_iv = y_true(:, k) - L1_use * z_k_now;
            % Σ̂_ε 更新: 滑动窗口协方差 (无遗忘)
            Sigma_eps_k = (e_k_iv * e_k_iv' + Sigma_eps_k * N_iv) / (N_iv + 1);
            % 简化: 用最近一拍残差 (与 Dinkla 论文一致)
            Sigma_eps_k = e_k_iv * e_k_iv' + 1e-3 * eye(p);  % 加小正则防止奇异
            Sigma_eps_k = (Sigma_eps_k + Sigma_eps_k') / 2;  % 对称化
        end
    end
